...
const AddTransactionForm = (props) => {
  ...
  const getProductDetail = () => {
    ...
    Axios.get(`/api/product_detail/${ref.current.values.product}`).then(
      (res) => {
  ...
  const validationSchema = Yup.object({
    ...
    quantity_of_product: Yup.number()
      .min(1)
      .max(1000000)
      .required(
        "Prosím zadejte počet prodaných produktů (minimálně 1 a maximálně 1 000 000 ks)"
      ),
  ...
  const initialValues = {
    day_of_sale: editedTransaction?.day_of_sale ?? formattedDate,
          ...
          <Form>
                    ...
                    <Grid item xs={12}>
                      ...
                      >
                        ...
                        {editedTransaction && (
                          <Typography>
                            Prodáno za cenu{" "}
                            <strong> {editedTransaction.real_price} Kč</strong>
                          </Typography>
                        )}
                      </Typography>
                    </Grid>
            ...
            {values.discount_increase !== "0" && <Grid item xs={4}>
              ...
              > 
                ...
                {values.discount_increase !== "0" && (
                  <Grid item xs={12}>
                    <TextField
                      name="difference_price"
                      // size="small"
                      label={
                        values.discount_increase === "-"
                          ? "Snížení ceny"
                          : "Navýšení ceny"
                      }
                      InputProps={{
                      ...
                      >
                        ...
                        >
                          {CurrentPrice(
                            productDetail.price,
                            values.discount_increase,
                            values.difference_price,
                            values.price_variant
                          )}
                        </Typography>
                        <Typography color="primary">
                          Skladem{" "}
